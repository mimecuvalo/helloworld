...
// Handlers are read at fire time, never captured, so a caller can swap them in
// place without tearing the listeners down.
type HandlersRef = { readonly current: GestureHandlers };
...
//
// The listeners live here, off React, so callers who don't render their target
// can use them too: feed HTML arrives as a blob through dangerouslySetInnerHTML,
// and its <img>s can only be reached by walking the DOM. Handlers come in behind
// a ref so inline closures don't force a re-bind on every render.
export function attachGestures(element: HTMLElement, handlersRef: HandlersRef) {
  let startX = 0;
  let startY = 0;
  let isSwiping = false;
...
// The React face of `attachGestures`.
...
export function useGestures(ref: RefObject<HTMLElement | null>, handlers: GestureHandlers, deps: unknown[] = []) {
  ...
  useEffect(() => {
    ...
    if (!element) return;
    return attachGestures(element, handlersRef);
