...
class MagicEnvelopeProtocol(object):
  ...
  def GetSignerURI(self, data):
    ...
    if isinstance(data, et.ElementTree):
      ...
      d = et.ElementTree()
      data = data.encode('utf8') if type(data) is unicode else data
  ...
  def ParseData(self, raw_text_data, mime_type):
    ...
    d = et.ElementTree()
    raw_text_data = raw_text_data.encode('utf8') if type(raw_text_data) is unicode else raw_text_data
  ...
  def Parse(self, textinput, mime_type='application/magic-envelope+xml'):
    ...
    textinput = textinput.encode('utf8') if type(textinput) is unicode else textinput
