...
export const POST = authenticate(async (request: NextRequest) => {
  try {
    ...
    const embed = await discoverAndRetrieveOEmbedOrOpenGraphDataFromUrl(body.url);
    if (!embed) {
    ...
    return NextResponse.json({
      success: true,
      wasMediaFound: !!(embed.image || embed.iframe),
      ...embed,
    });
  } catch (error) {
    console.error('Unfurl error:', error);
    return NextResponse.json({ success: false, error: 'Failed to unfurl URL' }, { status: 500 });
  }
});
