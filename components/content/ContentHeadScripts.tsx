...
export default function ContentHeadScripts(opts: { content: any; contentOwner: any; host: string; title: string }) {
  ...
  return (
    <>
      {scripts.map((s, i) =>
        s.children ? (
          <script key={i} type={s.type} suppressHydrationWarning dangerouslySetInnerHTML={{ __html: s.children }} />
        ) : s.src ? (
          <script key={i} src={s.src} async={s.async} suppressHydrationWarning />
        ) : null
      )}
    </>
  );
