...
def comment(handler, from_username, to_email, content_url, comment, this_user_mentioned=False):
  if this_user_mentioned:
    send(handler, handler.locale.translate('%(user)s made a post mentioning you in it!' % { "user": from_username }), to_email, handler.locale.translate(u"""
      <a href="%(url)s">View the post here!</a>
    """) % { "url": content_url })
  else:
    send(handler, handler.locale.translate('%(user)s made a comment on your post.' % { "user": from_username }), to_email, handler.locale.translate(u"""
...
def follow(handler, from_username, to_email, blog):
  ...
  send(handler, handler.locale.translate('%(user)s started following you!' % { "user": from_username }), to_email, handler.locale.translate(u"""
